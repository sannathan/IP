...
int main(void){
    ...
    for(int i = 0; i < n; i++){
        ...
        maisVotados[votos[i].agregados] += 1; //Vetor que contabiliza a quantidade de votos em cada respectivo numero
      
      if(i == 0){
        h = votos[i].agregados; //Se for a primeira entrada, a primeira posição do vetor maisVotados vai ser o primeiro valor de agregados recebido
      }
    ...
    int tam = 100;
    int maiorNum = maisVotados[h]; //O parametro de comparação vai ser a primeira quantidade de agregados recebido
    int maiorNum2 = h;
    for (int i = 0; i < tam; i++) {
      ...
      else if(maisVotados[i] == maiorNum){
        if(i < maiorNum){
            maiorNum = i;
        }
      }
      else{
        coordenada_maior_numero = maiorNum2;
      }
    }
